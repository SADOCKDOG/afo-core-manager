...
### Automated Budget Generation with BC3 Prices
- **Functionality**: Revolutionary automated budget creation system that leverages imported BC3 price databases (BEDEC, PREOC, BPHU) to generate complete construction budgets using three intelligent methods: manual price selection with search, predefined templates for common building types, and AI-powered automatic budget generation from project descriptions
- **Purpose**: Dramatically accelerate project quoting by reducing budget creation time from hours to minutes. Enable architects to generate accurate construction budgets immediately after importing BC3 price databases, eliminating manual price lookup and repetitive data entry. Particularly valuable for preliminary cost estimates during client meetings and proposal preparation.
- **Trigger**: User clicks "Generación Automática" button in Budget Manager (both from project list view and empty budget state)
- **Progression**:
  - **Manual Mode**: Open automated generator → Search price database (60,000+ prices from BEDEC/PREOC/BPHU) by code, description, or category → Click prices to add to budget → Set quantity for each price → Prices automatically organized by chapter → Adjust chapter assignments → Review estimated totals (PEM, GG+BI, IVA, Total) in real-time → Generate complete budget with hierarchical structure
  - **Template Mode**: Select building type template (Vivienda Unifamiliar, Rehabilitación, Local Comercial) → System automatically searches price database for compatible prices matching template categories (Movimiento de tierras, Cimentación, Estructura, Albañilería, Revestimientos, Carpintería, Instalaciones) → 20-40 relevant prices auto-selected → Review and adjust quantities → Generate budget instantly
  - **AI Mode**: Enter natural language project description ("Construcción de vivienda unifamiliar de 150m2 en dos plantas, estructura de hormigón, cubierta inclinada de teja, revestimientos de gres porcelánico...") → AI analyzes description and matches to price database codes → Generates intelligent quantity estimates based on surface areas and project type → Auto-assigns prices to logical chapters (01-10) → Review AI-selected prices and quantities → Adjust as needed → Generate complete budget with AI-recommended structure
- **Success criteria**: 
  - Manual mode allows instant search across entire imported BC3 database with real-time filtering; individual prices add with single click; quantity adjustments update totals immediately; estimator shows accurate PEM, GG+BI (19%), IVA (21%), and total
  - Template mode intelligently matches template categories to price database codes (handles variations in code prefixes); generates 15-30 partidas per template; organizes by logical chapters automatically; provides realistic starting point for common project types
  - AI mode accurately interprets Spanish architectural descriptions; matches project requirements to specific BC3 codes (not generic descriptions); generates contextually appropriate quantities based on stated surface areas and project scale; assigns intelligent chapter structure; achieves 60-70% accuracy on price selection requiring minimal manual adjustment
  - All modes create proper hierarchical budget structure with chapters, partidas, and calculated totals; generated budgets immediately editable in standard budget manager; supports any BC3 price database (BEDEC, PREOC, BPHU, custom imports); handles empty price database gracefully with clear guidance to import BC3 first; provides visual feedback during AI generation with progress indicators
