...
import { DocumentValidationPanel } from './Do
import { getRequiredDocuments } from '@/lib/visa-va
interface BoardPermitWorkflowProps {
}
export function BoardPermitWorkflow({ 
  const [visaApplications, set
  const [selectedVisa, setSelectedVisa] = useState<VisaApplicat

    ? (visaApplications || []).filter(v => v.projectId === 
  ...
  const handleAddDocument = (visaId: string, document: any) => {
    ...
    toast.success(`${document.name} añadido`)
  }
                ...
                <Tabs defaultValue="documents" className="flex-1">
                  ...
                  <TabsContent value="documents" className="flex-1 overflow-auto mt-4">
                    <DocumentValidationPanel
                      visa={selectedVisa}
                      onDocumentUpload={(doc) => handleAddDocument(selectedVisa.id, doc)}
                    />
                  </TabsContent>
